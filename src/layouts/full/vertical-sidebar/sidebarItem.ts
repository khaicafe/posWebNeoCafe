...
const mainItem: menu[] = [
    ...
    { header: 'Reports' },
    {
        title: 'Khách giới thiệu',
        icon: UsersGroupIcon,
        to: '/reports/total-customer-referrals'
    },
    {
        title: 'Khách hủy gia hạn',
        icon: UserCancelIcon,
        to: '/reports/total-customer-canceled'
    },
    {
        title: 'Tiền giới thiệu',
        icon: BusinessplanIcon,
        to: '/reports/money-referral-estimated'
    },
    {
        title: 'Tiền ship',
        icon: HomeDollarIcon,
        to: '/reports/money-ship-estimated'
    },
    { header: 'Voucher' },
    {
        title: 'Danh sách Voucher',
        icon: UserCancelIcon,
        to: '/voucher/list-voucher'
    },
    { header: 'Manager' },
    {
        title: 'Thông kê',
        icon: ChartPieIcon,
        to: '/dashboards/manager-staff',
        permission: 'GET:manager/reports'
    },
    {
        title: 'Danh sách nhân viên',
        icon: UsersGroupIcon,
        to: '/reports/listStaffs',
        permission: 'GET:manager/reports'
    }
    // {
    //     title: 'Reports Khách giới thiệu',
    //     icon: UsersGroupIcon,
    //     to: '/reports/total-customer-referrals'
    // },
    // {
    //     title: 'Reports Khách hủy gia hạn',
    //     icon: UserCancelIcon,
    //     to: '/reports/total-customer-canceled'
    // },
    // {
    //     title: 'Reports Tiền giới thiệu',
    //     icon: BusinessplanIcon,
    //     to: '/reports/money-referral-estimated'
    // },
    // {
    //     title: 'Reports Tiền ship',
    //     icon: HomeDollarIcon,
    //     to: '/reports/money-ship-estimated'
    // }
];

const templateItem: menu[] = [
    { header: 'Apps' },
    {
        title: 'Analytical',
        icon: ChartPieIcon,
...
const sidebarItem: menu[] = import.meta.env.PROD ? [...mainItem] : [...mainItem, ...templateItem];

export const getValidSidebarItems = async (can: any) => {
    const filteredValues = await Promise.all(
        sidebarItem.map(async (value) => {
            const result = value.permission === undefined ? true : await can(value.permission);
            return { value, result };
        })
    );
    const filteredArray = filteredValues.filter((item) => item.result);

    // Remove the header if its children are empty
    let items: menu[] = [];
    for (let i = 0; i < filteredArray.length; i++) {
        if (filteredArray[i].value.header && filteredArray[i + 1].value.header) {
            // items.splice(i, 1)
            continue;
        }
        items.push(filteredArray[i].value);
    }
